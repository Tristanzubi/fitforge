...
const TYPE_COLOR: Record<string, string> = {
  ...
  mobilite: "bg-app-deep text-app-fg2",
};
...
function BlocDetail({ bloc, onClose }: { bloc: Bloc; onClose: () => void }) {
  ...
  const textColor = BLOC_TEXT[bloc.couleur] ?? "text-app-fg1";
  const barColor = BLOC_BAR[bloc.couleur] ?? "bg-orange-400";
  const ringColor = BLOC_RING[bloc.couleur] ?? "ring-app-edge";
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 max-w-md mx-auto animate-in slide-in-from-bottom duration-300">
        <div className={`bg-app-card rounded-t-3xl ring-1 ${ringColor} max-h-[80vh] flex flex-col`}>
          {/* Handle */}
          <div className="flex justify-center pt-3 pb-1 flex-shrink-0">
            <div className="h-1 w-10 rounded-full bg-app-deep" />
          </div>

          {/* Header */}
          <div className="flex items-start justify-between px-5 pt-2 pb-4 flex-shrink-0">
            <div>
              <p className="text-[11px] text-app-fg3 font-medium uppercase tracking-wider mb-0.5">
                Bloc {bloc.numero} · Semaines {bloc.semaines}
              </p>
              <h2 className={`text-xl font-bold ${textColor}`}>{bloc.nom}</h2>
            </div>
            <button onClick={onClose} className="h-8 w-8 rounded-full bg-app-deep flex items-center justify-center hover:bg-app-deep transition-colors mt-1">
              <X className="h-4 w-4 text-app-fg2" />
            </button>
          </div>
          ...
          <div className="px-5 pb-4 flex-shrink-0">
            <div className="flex items-center justify-between text-xs mb-1.5">
              <span className="text-app-fg3">{done} séances complétées</span>
              <span className={`font-bold ${textColor}`}>{pct}%</span>
            </div>
            <div className="h-1.5 rounded-full bg-app-deep">
              <div className={`h-1.5 rounded-full ${barColor} transition-all`} style={{ width: `${pct}%` }} />
            </div>
          </div>

          <div className="w-full h-px bg-app-deep flex-shrink-0" />

          {/* Sessions list — scrollable */}
          <div className="overflow-y-auto flex-1 px-5 py-4 space-y-2 pb-6">
            <p className="text-[11px] font-semibold text-app-fg3 uppercase tracking-wider mb-3">
              ...
              return (
                <Link key={seance.id} href={`/seance/${seance.id}`} onClick={onClose}>
                  <div className={`rounded-2xl border p-3.5 transition-colors ${
                    isDone ? "border-green-500/20 bg-app-card/60 opacity-70"
                    : isSkipped ? "border-app-edge/50 bg-app-card/40 opacity-50"
                    : "border-app-edge bg-app-card hover:border-app-fg3"
                  }`}>
                    <div className="flex items-center gap-3">
                      <div className="flex flex-col items-center justify-center h-9 w-9 rounded-xl bg-app-deep shrink-0">
                        <span className="text-[8px] text-app-fg3 font-bold">{seance.jour.slice(0, 3).toUpperCase()}</span>
                        <span className="text-xs font-bold text-app-fg2">{i + 1}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <p className="text-sm font-semibold text-app-fg1 truncate">{seance.nom}</p>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-[10px] text-app-fg3">{seance.exercices.length} exercices</span>
                      ...
                      <ChevronRight className="h-4 w-4 text-app-fg3 shrink-0" />
                    </div>
                  </div>
                </Link>
              );
            ...
            <Link href={`/s-entrainer?bloc=${bloc.numero}`} onClick={onClose}>
              <div className={`mt-3 rounded-2xl border ${BLOC_RING[bloc.couleur] ?? "border-app-edge"} p-3.5 flex items-center justify-center gap-2 hover:opacity-80 transition-opacity`}>
  ...
  return (
    <>
      <button
        ...
        className="w-full text-left rounded-xl bg-app-card border border-app-edge p-3 flex items-center gap-3 hover:border-app-fg3 transition-colors active:scale-[0.99]"
      >
        <div className={`h-2.5 w-2.5 rounded-full flex-shrink-0 ${BLOC_BAR[bloc.couleur] ?? "bg-zinc-400"}`} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <span className="text-sm font-medium text-app-fg1">Bloc {bloc.numero} — {bloc.nom}</span>
            <span className="text-xs text-app-fg3">S{bloc.semaines}</span>
          </div>
          <div className="h-1 rounded-full bg-app-deep">
            <div className={`h-1 rounded-full ${BLOC_BAR[bloc.couleur] ?? "bg-orange-500"} transition-all`} style={{ width: `${pct}%` }} />
          </div>
        </div>
        <span className="text-xs text-app-fg3 flex-shrink-0">{done}/{total}</span>
      </button>
