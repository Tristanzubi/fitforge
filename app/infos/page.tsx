...
function Accordion({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="rounded-xl border border-app-edge bg-app-card overflow-hidden">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-4 py-3.5 text-left"
      >
        <span className="text-sm font-semibold text-app-fg1">{title}</span>
        {open
          ? <ChevronUp className="h-4 w-4 text-app-fg3 shrink-0" />
          : <ChevronDown className="h-4 w-4 text-app-fg3 shrink-0" />
        }
      </button>
      {open && (
        <div className="px-4 pb-4 text-sm text-app-fg2 leading-relaxed border-t border-app-edge pt-3 space-y-2">
          {children}
        </div>
      )}
    </div>
...
function Tag({ children, color = "zinc" }: { children: React.ReactNode; color?: "orange" | "green" | "blue" | "zinc" }) {
  const colors = {
    ...
    zinc: "bg-app-deep text-app-fg2 border-app-edge",
  };
  return (
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-orange-500/30 bg-orange-500/5 p-4 mb-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div>
            <p className="text-xl font-bold text-app-fg1">154g</p>
            <p className="text-[10px] text-app-fg3">Protéines/j</p>
          </div>
          <div>
            <p className="text-xl font-bold text-app-fg1">2.7L</p>
            <p className="text-[10px] text-app-fg3">Eau/j</p>
          </div>
          <div>
            <p className="text-xl font-bold text-app-fg1">~3000</p>
            <p className="text-[10px] text-app-fg3">kcal/j (aprx)</p>
          </div>
        </div>
      </div>

      <Accordion title="🥩 Protéines — pourquoi et combien ?">
        <p>Les protéines reconstruisent le muscle après l&apos;effort. Pour un rugbyman en préparation physique, <strong className="text-app-fg1">2 g/kg/j</strong> est le minimum — soit <strong className="text-app-fg1">154 g/jour</strong> pour toi.</p>
        <p className="mt-2"><strong className="text-app-fg1">Sources efficaces :</strong></p>
        <ul className="list-disc list-inside space-y-1 mt-1 text-app-fg2">
        ...
        <p className="mt-2 text-app-fg3 text-xs">💡 Répartis en 3-4 prises dans la journée — le corps ne stocke pas bien plus de ~40 g à la fois.</p>
      </Accordion>

      <Accordion title="🍚 Glucides — le carburant de la séance">
        <p>Les glucides alimentent l&apos;intensité. Pas ennemi — <strong className="text-app-fg1">allié essentiel</strong> pour les séances force et vitesse.</p>
        <p className="mt-2"><strong className="text-app-fg1">Avant la séance (1h30 - 2h avant) :</strong></p>
        ...
        <p className="mt-3"><strong className="text-app-fg1">Après la séance (dans les 45 min) :</strong></p>
        <ul className="list-disc list-inside space-y-1 mt-1">
          <li>Fenêtre anabolique : glucides rapides + protéines</li>
          <li>Ex : riz + poulet, pain + fromage blanc + banane, shake whey + banane</li>
        </ul>
        <p className="mt-3"><strong className="text-app-fg1">Jours cardio / footing :</strong></p>
        <p className="text-app-fg3">Légèrement moins de glucides si séance facile — maintiens les protéines.</p>
      </Accordion>

      <Accordion title="💧 Hydratation — souvent négligée">
        <p>Objectif de base : <strong className="text-app-fg1">35 ml/kg/j = 2.7 L</strong>. Ajoute <strong className="text-app-fg1">750 ml à 1 L</strong> par heure d&apos;entraînement.</p>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-blue-500/30 bg-blue-500/5 p-4 mb-4">
        ...
        <p className="text-sm text-app-fg2">Le <strong>chargeTarget</strong> est un point de départ, pas une obligation. Écoute ton corps — certains jours tu dépasses, d&apos;autres tu recules. Les deux sont normaux.</p>
      </div>
      ...
      <Accordion title="🤖 Règles de progression automatique">
        ...
        <div className="space-y-3 mt-2">
          <div className="rounded-lg bg-app-deep p-3">
            <p className="text-green-400 font-semibold text-xs mb-1">Progression +5%</p>
            <p>Si toutes les séries d&apos;un exercice sont en <Tag color="green">facile</Tag> lors de <strong className="text-app-fg1">2 séances consécutives</strong> → charge augmentée de 5%, arrondie à 0.5 kg près.</p>
          </div>
          <div className="rounded-lg bg-app-deep p-3">
          ...
          <div className="rounded-lg bg-app-deep p-3">
            <p className="text-app-fg2 font-semibold text-xs mb-1">Mise à jour neutre</p>
            <p>Sinon, le chargeTarget se met à jour avec la meilleure charge que tu as réellement utilisée ce jour-là.</p>
          </div>
        </div>
        <p className="mt-2 text-app-fg3 text-xs">⚡ Les exercices olympiques (KB clean, KB swing) et les exercices genou-sensibles ne sont jamais auto-ajustés.</p>
      </Accordion>
      ...
      <Accordion title="📈 Progression semaine par semaine">
        ...
        <div className="rounded-lg bg-app-deep p-3 mt-2 font-mono text-xs text-app-fg2">
        ...
        <p className="mt-2 text-app-fg3">Navigue entre les semaines dans la page S&apos;entraîner avec les flèches ← →.</p>
      </Accordion>

      <Accordion title="⚠️ Quand reculer ou s'arrêter ?">
        <p><strong className="text-app-fg1">Fatigue musculaire normale</strong> = courbatures, muscles fatigués 24-48h après. Tu continues.</p>
        ...
        <p className="mt-2 text-app-fg3">En cas de doute, utilise le Coach Claude — décris la douleur et il t&apos;orientera.</p>
      </Accordion>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-green-500/30 bg-green-500/5 p-4 mb-4">
        ...
        <p className="text-sm text-app-fg2">Si un exercice est trop dur ou trop facile, utilise une variante plutôt que de sauter la séance. La progression est linéaire — tu y arriveras.</p>
      </div>
      ...
      <Accordion title="⚡ KB clean — technique clé">
        ...
        <ul className="list-disc list-inside space-y-1 mt-2">
          <li><strong className="text-app-fg1">Départ :</strong> KB entre les jambes, dos plat, prise neutre</li>
          <li><strong className="text-app-fg1">Phase pull :</strong> pousser le sol, extension rapide hanches + genoux</li>
          <li><strong className="text-app-fg1">Phase catch :</strong> coude qui descend vite sous le KB, poignet en supination, KB contre l&apos;avant-bras (pas dans la paume)</li>
          <li><strong className="text-app-fg1">Erreur fréquente :</strong> tirer avec les bras → le KB part trop loin, tu t&apos;écorches le poignet</li>
        </ul>
        <p className="mt-2 text-app-fg3">Si tu t&apos;écorches le poignet → la trajectoire est trop loin du corps. Garde le KB proche.</p>
      </Accordion>
      ...
      <Accordion title="🏃 KB swing — 2 styles">
        <div className="space-y-3">
          ...
          <p><strong className="text-app-fg1">Clé du swing :</strong> le mouvement vient des hanches (hip hinge), pas du dos ni des bras. Si tu ressens le dos → réduire la charge et retravailler la technique.</p>
        </div>
      </Accordion>

      <Accordion title="🤸 Bulgarian split squat — astuces">
        <ul className="list-disc list-inside space-y-1">
          <li><strong className="text-app-fg1">Pied arrière :</strong> pointe du pied sur la surface — si douleur genou avant, essaie le dessus du pied à plat</li>
          <li><strong className="text-app-fg1">Distance :</strong> jambe avant assez loin pour que le genou ne dépasse pas trop le pied à la descente</li>
          <li><strong className="text-app-fg1">Difficulté :</strong> plus la surface est haute, plus c&apos;est difficile. Commence avec une chaise basse (40 cm), monte à canapé (50 cm)</li>
          <li><strong className="text-app-fg1">Charge :</strong> haltères tenus en bas (neutre) {`>`} KB en goblet {`>`} barre (indisponible ici)</li>
        </ul>
      </Accordion>
      ...
      <Accordion title="🤲 Turkish get-up — apprendre par étapes">
        ...
        <p className="mt-2 text-app-fg3">Commence avec un verre d&apos;eau (sans le renverser !) ou KB 8-12 kg avant d&apos;utiliser KB 16-24 kg. Regard sur le KB pendant tout le mouvement.</p>
      </Accordion>

      <Accordion title="🔄 Si tu n'as pas le bon matériel">
        <div className="space-y-2">
          <div className="flex gap-2">
            <span className="text-app-fg3 text-xs shrink-0 w-32">KB clean KB24</span>
            <span className="text-xs">→ KB20 avec 1-2 reps de moins, ou 2 KB20 en double clean</span>
          </div>
          <div className="flex gap-2">
            <span className="text-app-fg3 text-xs shrink-0 w-32">Farmer&apos;s carry</span>
            <span className="text-xs">→ Les 2 KB en même temps (KB24 + KB20 = 44 kg total) ou sac à dos lesté</span>
          </div>
          <div className="flex gap-2">
            <span className="text-app-fg3 text-xs shrink-0 w-32">Dips chaises</span>
            <span className="text-xs">→ 2 chaises solides ou bord du canapé — vérifier la stabilité avant de charger</span>
          </div>
          <div className="flex gap-2">
            <span className="text-app-fg3 text-xs shrink-0 w-32">Step-up</span>
            <span className="text-xs">→ Chaise solide (50 cm) ou première marche d&apos;escalier</span>
          </div>
          <div className="flex gap-2">
            <span className="text-app-fg3 text-xs shrink-0 w-32">Tractions lestées</span>
  ...
  return (
    <div className="min-h-screen pb-28 px-4 py-6 max-w-lg mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link
          href="/dashboard"
          className="flex items-center justify-center h-8 w-8 rounded-lg bg-app-card border border-app-edge hover:border-app-fg3 shrink-0"
        >
          <ChevronLeft className="h-4 w-4 text-app-fg2" />
        </Link>
        <div>
          <h1 className="text-lg font-bold text-app-fg1">Infos & Conseils</h1>
          <p className="text-xs text-app-fg3">Nutrition · Charges · Variantes</p>
        </div>
      </div>
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-semibold border transition-all ${
              activeTab === id
                ...
                : "border-app-edge bg-app-card text-app-fg3 hover:text-app-fg1"
            }`}
          >
